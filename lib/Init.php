    ...
    if (!$acfActive) {
      ...
      add_filter('template_include', function (): void {
        ...
        $message = sprintf(
          ...
          esc_html__(
            'Please %1$sactivate or install the required plugin(s)%2$s and reload the page.',
            'flynt',
          ),
  ...
  protected static function notifyAcfPluginIsMissing()
  {
    add_action('admin_notices', function (): void {
      ...
      $message = sprintf(
        ...
        esc_html__(
          '%1$s plugin not activated. Make sure you activate the plugin on the %2$s.',
          'flynt',
        ),
        $pluginName,
        $pluginsUrl,
      ...
      printf(
        '<div class="%1$s"><p><strong>%2$s</strong></p><p>%3$s</p></div>',
        $class,
        $title,
        $message,
      );
