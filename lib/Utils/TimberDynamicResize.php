...
class TimberDynamicResize
{
  ...
  protected function addHooks()
  {
    add_action('timber/twig', function ($twig) {
      $twig->addFilter(new TwigFilter('resizeDynamic', [$this, 'resizeDynamic']));
      return $twig;
    });
    if ($this->enabled) {
  ...
  public function checkAndGenerateImage(string $relativePath): void
  {
    ...
    if ($exists) {
      ...
      $resizedImage = $wpdb->get_row(
        $wpdb->prepare(
          "SELECT * FROM {$tableName} WHERE width = %d AND height = %d AND crop = %s",
          [$w, $h, $crop],
        ),
      );
    }
  ...
  public function addImageSeparatorToUploadUrl(string $url): string
  {
    ...
    return str_replace($baseurl, trailingslashit($baseurl) . self::IMAGE_PATH_SEPARATOR, $url);
  }
