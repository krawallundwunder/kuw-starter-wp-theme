...
class ComponentManager
{
  ...
  public function registerComponent(string $componentName, ?string $componentPath = null): bool
  {
    ...
    $componentPath = trailingslashit(
      apply_filters('Flynt/componentPath', $componentPath, $componentName),
    );
  ...
  public function get(string $componentName)
  {
    ...
    if (!$this->isRegistered($componentName)) {
      trigger_error(
        "Cannot get component: Component '{$componentName}' is not registered!",
        E_USER_WARNING,
      );
      return false;
    }
  ...
  public function getComponentsWithScript(): array
  {
    ...
    foreach ($this->components as $componentName => $componentPath) {
      ...
      $relativeComponentPath = trim(
        str_replace(get_template_directory() . '/Components/', '', $componentPath),
        '/',
      );
