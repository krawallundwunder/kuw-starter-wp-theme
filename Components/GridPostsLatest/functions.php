...
add_filter('Flynt/addComponentData?name=GridPostsLatest', function (array $data): array {
  ...
  $data['posts'] = array_slice(
    array_filter($posts->to_array(), function ($post): bool {
  ...
  $data['postTypeArchiveLink'] =
    get_permalink(get_option('page_for_posts')) ?? get_post_type_archive_link(POST_TYPE);
  ...
  return [
    ...
    'sub_fields' => [
      ...
      [
        ...
        'instructions' => __(
          'Select 1 or more categories or leave empty to show from all posts.',
          'flynt',
        ),
        'name' => 'taxonomies',
        'type' => 'taxonomy',
