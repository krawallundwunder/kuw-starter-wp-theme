...
if (!function_exists('phmu_starter_wp_theme_setup')) :
	...
	function phmu_starter_wp_theme_setup()
	{
		 ...
		 */
		load_theme_textdomain('phmu-starter-wp-theme', get_template_directory() . '/languages');
		...
		add_theme_support(
			'custom-background',
			apply_filters(
				'phmu_starter_wp_theme_custom_background_args',
				array(
 ...
 */
function phmu_starter_wp_theme_content_width()
{
	$GLOBALS['content_width'] = apply_filters('phmu_starter_wp_theme_content_width', 640);
}

add_action('after_setup_theme', 'phmu_starter_wp_theme_content_width', 0);
...
{
	wp_enqueue_style('phmu-starter-wp-theme-style', get_template_directory_uri() . '/style.min.css', array(), _S_VERSION);

	wp_enqueue_script('phmu-starter-wp-theme-navigation', get_template_directory_uri() . '/assets/js/custom.min.js', array(), _S_VERSION, true);
