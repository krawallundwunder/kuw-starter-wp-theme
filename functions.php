...
function phmu_disable_editor($id = false)
{
 ...
 *
 */
function phmu_disable_gutenberg($can_edit, $post_type)
{
  if (!(is_admin() && !empty($_GET['post']))) {
    return $can_edit;
  }

  if (phmu_disable_editor($_GET['post'])) {
    $can_edit = false;
  }

  return $can_edit;
}
add_filter('gutenberg_can_edit_post_type', 'phmu_disable_gutenberg', 10, 2);
add_filter('use_block_editor_for_post_type', 'phmu_disable_gutenberg', 10, 2);
